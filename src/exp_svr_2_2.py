def predictionModel(n_days, file_path):
    ...
    future_days_df = pd.DataFrame(future_days, columns=['Days'])  # Convert to DataFrame with column name
    ...
    fig.add_trace(
        go.Scatter(
            x=future_dates,
            y=svr_model.predict(future_days_df),  # Use DataFrame for prediction
            mode='lines+markers',
